...
SCON_SS_priors_details = {k: v.to(active_device) for k, v in torch.load(os.path.join('generated_data/SCON-SS_CO2_logit_short/', 'p_theta.pt')).items()}

#Initial condition prior means
x0_SCON_tensor = torch.load(os.path.join('generated_data/SCON-SS_CO2_logit_short/', 'p_x0.pt')).to(active_device)
...
csv_data_path = os.path.join('generated_data/SCON-SS_CO2_logit_short/', 'data.csv')
...
#true_theta = torch.load(os.path.join('generated_data/SCON-SS_CO2_logit_short/', 'theta.pt'), map_location = active_device)
