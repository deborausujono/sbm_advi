...
###############################
def eval_elbo(eval_batch_size, net, q_theta, p_theta, obs_model, fix_theta_dict, dt_flow, N, SBM_SDE_instance, x0_prior, learn_CO2):
    net.eval()
    with torch.no_grad():
...
def train(DEVICE, ELBO_LR: float, ELBO_ITER: int, BATCH_SIZE: int,
          ...
          PRINT_EVERY: int = 100, EVAL_ELBO_EVERY: int = 100, EVAL_BATCH_SIZE: int = 100, VERBOSE = False,
        ...
        for iteration in range(T_ITER + 1):
            if iteration % EVAL_ELBO_EVERY == 0:
                ...
                if (iteration + 1) % PRINT_EVERY == 0:
                    print(f'\nTest ELBO at {iteration} iterations = {ELBO_losses[-1]}')
